...
Training::Training(QWidget *parent)
    : QWidget(parent), ui_(new Ui::Training), graph_data_size_(1) {
  ui_->setupUi(this);
...
TrainingMode Training::GetMode() { return current_mode_; }

size_t Training::GetEpochs() { return ui_->spin_box_groups_or_epochs->value(); }

size_t Training::GetGroups() { return ui_->spin_box_groups_or_epochs->value(); }
...
void Training::OpenTestingFile() { OpenNewFile(ui_->label_testing_file_path); }

void Training::OpenNewFile(QLabel *field) {
  QString file_path = QFileDialog::getOpenFileName(this, "Open .csv file",
                                                   "/Users", "csv (*.csv);;");
...
void Training::SetGraphRangeH(int value) { errors_graph_->SetRangeX(0, value); }
...
void Training::MakeFileName() {
  QDateTime date_time;
  saving_path_.append(
      "/MLP_" + date_time.currentDateTime().toString("yyyy_MM_dd_hh_mm_ss") +
      "_");
}
...
void Training::SetupConnections() {
  ...
  connect(ui_->btn_open_training_file, &QPushButton::clicked, this,
          &Training::OpenTrainingFile);
  connect(ui_->btn_open_testing_file, &QPushButton::clicked, this,
          &Training::OpenTestingFile);
  connect(ui_->combo_training_mode, &QComboBox::currentIndexChanged, this,
          &Training::ChangeTrainingMode);
  connect(ui_->btn_training_run, &QPushButton::clicked, this, &Training::Start);
  connect(ui_->btn_training_terminate, &QPushButton::clicked, this,
          &Training::Stop);
  connect(&process_timer_, &QTimer::timeout, this, &Training::IncreaseTimer);
  connect(ui_->spin_box_groups_or_epochs, &QSpinBox::valueChanged, this,
          &Training::SetGraphRangeH);
  connect(ui_->btn_save_weights, &QPushButton::clicked, this,
          &Training::ChooseSavingDir);
  connect(ui_->spin_box_learning_rate, &QDoubleSpinBox::valueChanged, this,
          &Training::UpdateLearningRateSignal);
}
