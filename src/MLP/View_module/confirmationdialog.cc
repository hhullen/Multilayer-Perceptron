...
ConfirmationDialog::ConfirmationDialog(QWidget *parent, QString question) :
    QDialog(parent),
    ui_(new Ui::ConfirmationDialog) {
    ui_->setupUi(this);
    ui_->label_question->setText(question);
    setWindowTitle("");

    connect(ui_->btn_accept, &QPushButton::clicked, this, &ConfirmationDialog::SetAcceted);
    connect(ui_->btn_reject, &QPushButton::clicked, this, &ConfirmationDialog::SetRejected);
}
