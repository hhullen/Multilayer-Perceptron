...
Classifier::Classifier(QWidget *parent)
  ...
  connect(ui_->btn_back_menu, &QPushButton::clicked, this,
          &Classifier::BackSignal);
  connect(ui_->combo_box, &QComboBox::currentIndexChanged, this,
          &Classifier::ChangeMode);
  connect(btn_open_img_, &QPushButton::clicked, this,
          &Classifier::OpenBMPImage);
  connect(btn_close_img_, &QPushButton::clicked, this,
          &Classifier::CloseBMPImage);
}
...
void Classifier::ProcessPicture() {
  ...
  input_screen.save("/Users/hhullen/PROJECTS/CPP7_MLP-0/src/screen.bmp", "BMP");
  input_screen = input_screen.scaled(
      kIMAGE_SIZE, kIMAGE_SIZE, Qt::AspectRatioMode::KeepAspectRatio,
...
void Classifier::OpenBMPImage() {
  QString file_path = QFileDialog::getOpenFileName(this, "Open .bmp file",
                                                   "/Users", "bmp (*.bmp);;");
  if (!file_path.isEmpty()) {
    file_name_->setText(file_path);
    input_image_.load(file_path);
    if (input_image_.height() != kBMP_IMAGE_ZISE &&
        input_image_.width() != kBMP_IMAGE_ZISE) {
      CloseBMPImage();
      emit SentMessage(
          "Can't open the picture with size different from 512x512");
      file_name_->clear();
    } else {
      image_->setPixmap(QPixmap::fromImage(input_image_));
      btn_open_img_->setVisible(false);
      btn_close_img_->setVisible(true);
      input_image_ = input_image_.scaled(
          kIMAGE_SIZE, kIMAGE_SIZE, Qt::AspectRatioMode::KeepAspectRatio,
          Qt::TransformationMode::SmoothTransformation);
